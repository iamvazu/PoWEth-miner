var INFURA_ROPSTEN_URL = 'https://ropsten.infura.io/anb94achAHWObifKnoZ7';
var INFURA_MAINNET_URL = 'https://mainnet.infura.io/anb94achAHWObifKnoZ7';
/*
...
var tokenContractJSON = require('../contracts/_PoWEthToken.json');
...
module.exports =  {
    ...
    async init(web3,miningLogger)
    {
     ...
     if(isUnlocked){
       ...
       if( !this.encryptedStorageExists() )
      {
        ...
        mkdirp(this.getpowethLocalFolderPath(), function (err) {
            if (err) console.error(err)
            else console.log('created new encrypted keystore file at ' + this.getpowethLocalFolderPath())
        }.bind(this));

        localStorage = new LocalStorage(this.getpowethLocalFolderPath()+'/POWEminerstore');
    ...
    getpowethLocalFolderPath()
    {
      return this.getOSLocalDataFolderPath()  + '/.poweth';
    },
   ...
   async handleAccountCommand(subsystem_command,subsystem_option)
    ...
    encryptedStorageExists()
    {
      ...
      if (!fs.existsSync(this.getpowethLocalFolderPath()) || !fs.existsSync(this.getpowethLocalFolderPath() +'/POWEminerstore' )  ){
        return false;
      }

      if (!fs.existsSync(this.getpowethLocalFolderPath()+'/POWEminerstore/vaultdata' ) ){
        return false;
      }
    ...
    getDefaultContractAddress()
    {
      ...
      return ContractInterface.networks.mainnet.contracts._PoWEthToken.blockchain_address
    },
