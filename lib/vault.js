...
module.exports =  {
    ...
    async init(web3,miningLogger)
    {
     ...
     if(isUnlocked){
       ...
       if( !this.encryptedStorageExists() )
      {
        ...
        localStorage = new LocalStorage(this.getpowethLocalFolderPath()+'/0xbtcminerstore');
   ...
   async handleAccountCommand(subsystem_command,subsystem_option)
    ...
    encryptedStorageExists()
    {
      ...
      if (!fs.existsSync(this.getpowethLocalFolderPath()) || !fs.existsSync(this.getpowethLocalFolderPath() +'/0xbtcminerstore' )  ){
        return false;
      }

      if (!fs.existsSync(this.getpowethLocalFolderPath()+'/0xbtcminerstore/vaultdata' ) ){
        return false;
      }
